...
control '4.1.6_Ensure_events_that_modify_date_and_time_information_are_collected' do
  ...
  impact 1.0
  describe auditd do
    its('lines') { should include(/-a always,exit -F arch=b32 -S clock_settime -F key=time-change/) }
    its('lines') { should include(/-a always,exit -F arch=b32 -S stime,settimeofday,adjtimex -F key=time-change/) }
  ...
  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include(/-a always,exit -F arch=b64 -S clock_settime -F key=time-change/) }
      its('lines') { should include(/-a always,exit -F arch=b64 -S adjtimex,settimeofday -F key=time-change/) }
    end
    describe file('/etc/audit/audit.rules') do
...
control '4.1.8_Ensure_events_that_modify_the_systems_network_environment_are_collected' do
  ...
  impact 1.0
  describe auditd do
    its('lines') { should include(/-a always,exit -F arch=b32 -S sethostname,setdomainname -F key=system-locale/) }
  ...
  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include(/-a always,exit -F arch=b64 -S sethostname,setdomainname -F key=system-locale/) }
    end
    describe file('/etc/audit/audit.rules') do
...
control '4.1.9_Ensure_discretionary_access_control_permission_modification_events_are_collected' do
  ...
  impact 1.0
  describe auditd do
    its('lines') { should include(/-a always,exit -F arch=b32 -S setxattr,lsetxattr,fsetxattr,removexattr,lremovexattr,fremovexattr -F auid>=1000 -F auid!=-1 -F key=perm_mod/) }
    its('lines') { should include(/-a always,exit -F arch=b32 -S lchown,fchown,chown,fchownat -F auid>=1000 -F auid!=-1 -F key=perm_mod/) }
    its('lines') { should include(/-a always,exit -F arch=b32 -S chmod,fchmod,fchmodat -F auid>=1000 -F auid!=-1 -F key=perm_mod/) }
  end
  describe file('/etc/audit/audit.rules') do
  ...
  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include(/-a always,exit -F arch=b64 -S setxattr,lsetxattr,fsetxattr,removexattr,lremovexattr,fremovexattr -F auid>=1000 -F auid!=-1 -F key=perm_mod/) }
      its('lines') { should include(/-a always,exit -F arch=b64 -S chown,fchown,lchown,fchownat -F auid>=1000 -F auid!=-1 -F key=perm_mod/) }
      its('lines') { should include(/-a always,exit -F arch=b64 -S chmod,fchmod,fchmodat -F auid>=1000 -F auid!=-1 -F key=perm_mod/) }
    end
    describe file('/etc/audit/audit.rules') do
...
control '4.1.10_Ensure_unsuccessful_unauthorized_file_access_attempts_are_collected' do
  ...
  impact 1.0
  describe auditd do
    its('lines') { should include(/-a always,exit -F arch=b32 -S open,creat,truncate,ftruncate,openat -F exit=-EPERM -F auid>=1000 -F auid!=-1 -F key=access/) }
    its('lines') { should include(/-a always,exit -F arch=b32 -S open,creat,truncate,ftruncate,openat -F exit=-EACCES -F auid>=1000 -F auid!=-1 -F key=access/) }
  end
  describe file('/etc/audit/audit.rules') do
  ...
  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include(/-a always,exit -F arch=b64 -S open,truncate,ftruncate,creat,openat -F exit=-EPERM -F auid>=1000 -F auid!=-1 -F key=access/) }
      its('lines') { should include(/-a always,exit -F arch=b64 -S open,truncate,ftruncate,creat,openat -F exit=-EACCES -F auid>=1000 -F auid!=-1 -F key=access/) }
    end
    describe file('/etc/audit/audit.rules') do
...
control '4.1.12_Ensure_successful_file_system_mounts_are_collected' do
  ...
  impact 1.0
  describe auditd do
    its('lines') { should include(/-a always,exit -F arch=b32 -S mount -F auid>=1000 -F auid!=-1 -F key=mounts/) }
  end
  describe file('/etc/audit/audit.rules') do
  ...
  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include(/-a always,exit -F arch=b64 -S mount -F auid>=1000 -F auid!=-1 -F key=mounts/) }
    end
    describe file('/etc/audit/audit.rules') do
...
control '4.1.13_Ensure_use_of_privileged_commands_is_collected' do
  ...
  command('find / -xdev \\( -perm -4000 -o -perm -2000 \\) -type f').stdout.split("\n").each do |privileged_command|
    describe auditd do
      its('lines') { should include /-a always,exit -S all -F path=#{privileged_command} -F perm=x -F auid>=1000 -F auid!=-1 -F key=privileged/ }
    end
  end
end
...
control '4.1.14_Ensure_file_deletion_events_by_users_are_collected' do
  ...
  impact 1.0
  describe auditd do
    its('lines') { should include(/-a always,exit -F arch=b32 -S unlink,rename,unlinkat,renameat -F auid>=1000 -F auid!=-1 -F key=delete/) }
  end
  describe file('/etc/audit/audit.rules') do
  ...
  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include(/-a always,exit -F arch=b64 -S rename,unlink,unlinkat,renameat -F auid>=1000 -F auid!=-1 -F key=delete/) }
    end
    describe file('/etc/audit/audit.rules') do
...
control '4.1.15_Ensure_kernel_module_loading_and_unloading_is_collected' do
  ...
  describe auditd do
    ...
    its('lines') { should include(/-a always,exit -F arch=b32 -S init_module,delete_module -F key=modules/) }
  end
  describe file('/etc/audit/audit.rules') do
  ...
  if command('uname -m').stdout.strip == 'x86_64'
    describe auditd do
      its('lines') { should include(/-a always,exit -F arch=b64 -S init_module,delete_module -F key=modules/) }
    end
    describe file('/etc/audit/audit.rules') do
