...
control '1.1.23_Disable_USB_Storage' do
  ...
  describe.one do
    a.each do |entry|
      describe entry do
        it { should match(%r{^install\s+/bin/true\s*$}) }
      end
    end
  end
end
...
control '1.5.3_Ensure_authentication_required_for_single_user_mode' do
  ...
  describe file('/usr/lib/systemd/system/emergency.service') do
    its('content') { should match(%r{^\s*ExecStart=-/usr/lib/systemd/systemd-sulogin-shell(\s+emergency|\s*)\s*(\s+#.*)?$}) }
  end
  describe file('/usr/lib/systemd/system/rescue.service') do
    its('content') { should match(%r{^\s*ExecStart=-/usr/lib/systemd/systemd-sulogin-shell(\s+rescue\s*|\s*)\s*(\s+#.*)?$}) }
  end
end
...
control '1.8.2_Ensure_GDM_login_banner_is_configured' do
  ...
  describe.one do
    describe file('/etc/gdm/greeter.dconf-defaults') do
      it { should exist }
      its('content') { should match(%r{^\[org/gnome/login-screen\]$}) }
